<template>
    <div class="p-8 h-full relative">
        <div class="flex items-center">
            ...
            <h2 class="ml-3 font-medium text-gray-800">{{current_user.privat_key}}</h2>
        </div>
        

        <!-- Loading -->
        <div class="h-3/4 flex item-center" v-if="isLoading">
...
export default {
    ...
    mounted() {
        ...
        window.Echo.channel('message.'+ this.conversation.id).listen('MessageCreated', (event) => {
            ...
            this.conversation.message.push({ body: event.message.body})
        })
